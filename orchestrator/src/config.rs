...
pub struct Config {
    ...
    /// across orchestrator restarts. Defaults to `./vm-backends.json`.
    pub vm_backends_file: PathBuf,
    /// Path to the JSON file where volume_id → backend_url mappings are persisted
    /// across orchestrator restarts. Defaults to `./volume-backends.json`.
...
impl Config {
    pub fn load() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let orchestrator_port = env::var("ORCHESTRATOR_PORT")
...
mod tests {
    ...
    #[test]
    fn test_default_orchestrator_port() {
        let _g = env_guard();
    ...
    fn test_orchestrator_port_from_env() {
        let _g = env_guard();
        env::set_var("ORCHESTRATOR_PORT", "9090");
        let config = Config::load().unwrap();
        env::remove_var("ORCHESTRATOR_PORT");
        assert_eq!(config.orchestrator_port, 9090);
    }
    ...
    fn test_invalid_port_falls_back_to_3000() {
        let _g = env_guard();
        env::set_var("ORCHESTRATOR_PORT", "not_a_number");
        let config = Config::load().unwrap();
        env::remove_var("ORCHESTRATOR_PORT");
