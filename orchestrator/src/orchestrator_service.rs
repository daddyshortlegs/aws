...
impl OrchestratorService {
    ...
    /// Forward request to any available backend via the registry.
    ...
    /// Forward request to an explicit backend URL; returns a concrete Response so callers
    /// can inspect the status and body before forwarding (e.g. launch_vm_handler).
    pub async fn forward_request_to(
        &self,
        backend_url: String,
        method: Method,
        uri: Uri,
    ...
    pub async fn list_all(&self, path: &str, headers: HeaderMap) -> impl IntoResponse {
        ...
        // set by the backend; the orchestrator (which runs on the controller alongside
    ...
    async fn forward(
        ...
        info!("Forwarding {} {} -> {}", method, path, backend_url);
...
mod tests {
    ...
    fn make_service(backend_url: Option<String>) -> OrchestratorService {
    ...
    fn service_for_port(port: u16) -> OrchestratorService {
    ...
    async fn test_transfer_encoding_absent_from_proxied_response() {
        ...
        // response the orchestrator delivers to the client.
    ...
    async fn test_backend_url_updated_dynamically() {
        ...
        // Before registration the orchestrator must refuse requests.
        let resp = svc
            .forward_request(
                Method::GET,
                "/".parse::<Uri>().unwrap(),
    ...
    async fn test_list_all_resolves_mac_address_to_ssh_host() {
        ...
        assert_eq!(
            ...
            Some("10.0.0.188"),
            "orchestrator should resolve mac_address to ssh_host via lease file"
        );
    }
