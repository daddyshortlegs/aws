...
use config::Config;
use orchestrator_service::OrchestratorService;
...
// orchestrator itself forwards JSON opaquely and never constructs these types at runtime.
...
struct VmListEntry {
    id: String,
    name: String,
    /// SSH host to connect to. Resolved from the dnsmasq lease file by the orchestrator in bridge mode.
    ssh_host: String,
    ssh_port: u16,
    pid: u32,
...
async fn launch_volume_handler(
    ...
    let response = state
        ...
        .forward_request_to(backend_url.clone(), method, uri, headers, body, None)
        .await;
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "Volume ID not known to this orchestrator"),
    ),
    tag = "volumes"
)]
...
async fn list_volume_files_handler(
    ...
    state
        .orchestrator_service
        .forward_request_to(backend_url, method, uri, headers, None, None)
        .await
        .into_response()
}
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid request body"),
        (status = 404, description = "VM ID not known to this orchestrator"),
...
async fn main() {
    ...
    tracing::info!("Starting orchestrator server with config: {:?}", config);

    let registry: Arc<RwLock<BackendRegistry>> = Arc::new(RwLock::new(BackendRegistry::new()));

    // Restore vm_id → backend_url mappings saved before the last orchestrator restart.
    ...
    // Restore volume_id → backend_url mappings saved before the last orchestrator restart.
    ...
    let listener =
        tokio::net::TcpListener::bind(format!("{}:{}", config.listen_ip, config.orchestrator_port))
            .await
            .unwrap();

    tracing::info!(
        "Orchestrator server listening on {}",
...
async fn forward_handler(
    ...
    state
        .orchestrator_service
        .forward_request(method, uri, headers, body, None)
        .await
}
...
mod tests {
    ...
    async fn test_delete_vm_removes_mapping_from_file() {
        ...
        // Register the backend so the orchestrator can route to it.
        app.clone()
            .oneshot(
                Request::builder()
                    .method("POST")
